(() => {
  ...
  const ui={overlay:$('overlay'),art:$('overlayArt'),kicker:$('overlayKicker'),title:$('overlayTitle'),body:$('overlayBody'),modes:$('modePicker'),start:$('startBtn'),hint:$('overlayHint'),time:$('timeLabel'),score:$('scoreLabel'),rank:$('rankLabel'),combo:$('comboLabel'),leaderboard:$('leaderboard'),dash:$('dashBtn'),touchDash:$('touchDash'),dashStatus:$('dashStatus'),pause:$('pauseBtn'),sound:$('soundBtn'),toast:$('toast'),event:$('eventBanner'),mission:$('missionLabel')};
  ...
  const modes={party:{name:'派对乱斗',time:60,bots:3,interval:13},duo:{name:'双人同屏',time:60,bots:2,interval:13},rush:{name:'闪电狂欢',time:40,bots:3,interval:9}};
  ...
  function beep(freq=550,duration=.13,type='sine',volume=.055){if(muted||!music.paused)return;try{audio||=new(window.AudioContext||window.webkitAudioContext)();if(audio.state==='suspended')audio.resume();const o=audio.createOscillator(),g=audio.createGain(),t=audio.currentTime;o.type=type;o.frequency.setValueAtTime(freq,t);o.frequency.exponentialRampToValueAtTime(Math.max(80,freq*.72),t+duration);g.gain.setValueAtTime(volume,t);g.gain.exponentialRampToValueAtTime(.001,t+duration);o.connect(g).connect(audio.destination);o.start(t);o.stop(t+duration)}catch(_){}}
  function fanfare(){[523,659,784,1046].forEach((n,i)=>setTimeout(()=>beep(n,.18,'triangle'),i*82))}
  function say(text){ui.toast.textContent='✨✨✨';ui.toast.classList.add('show');clearTimeout(toastTimer);toastTimer=setTimeout(()=>ui.toast.classList.remove('show'),1300)}
  ...
  function actor(name,id,x,y,bot=false,second=false){return{name,id,x,y,bot,second,score:0,combo:0,comboTime:0,pickupCooldown:0,dash:0,cool:0,stun:0,invuln:0,lastDx:0,lastDy:-1,hit:new Set(),radius:22,think:0,target:null}}
  ...
  function collide(a,b){if(a.dash<=0||a.hit.has(b)||b.invuln>0)return;const stolen=Math.min(b.score,30+Math.floor(a.combo*5));if(!stolen||b===game.player)return;a.hit.add(b);a.score+=stolen;b.score-=stolen;b.stun=.65;b.invuln=1.2;game.shake=.15;burst(b.x,b.y,22,['#fff3ab','#ff958b','#b9d7ff']);floatText(b.x,b.y,`-${stolen}`,'#ff9eaa');floatText(a.x,a.y,`+${stolen}`,'#ffdf8b');if(!a.bot)say(`⚡ 加速成功！+${stolen}`);beep(240,.17,'sawtooth',.05)}
  ...
  function updateActor(a,dt){a.comboTime=Math.max(0,a.comboTime-dt);if(!a.comboTime)a.combo=0;a.pickupCooldown=Math.max(0,a.pickupCooldown-dt);a.cool=Math.max(0,a.cool-dt);a.dash=Math.max(0,a.dash-dt);a.stun=Math.max(0,a.stun-dt);a.invuln=Math.max(0,a.invuln-dt);if(a.stun>0)return;let dx=0,dy=0;
    if(a.bot){
      // Friends give beginners time to choose and leave their nearby cakes alone.
      if(game.elapsed<5||a.pickupCooldown>0)return;
      a.think-=dt;
      if(a.think<=0||!game.cakes.includes(a.target)){
        a.think=rand(.8,1.4);
        const choices=game.cakes.filter(c=>dist(c,game.player)>150&&(!pointer.enabled||dist(c,pointer)>85));
        a.target=choices.reduce((best,c)=>!best||dist(a,c)<dist(a,best)?c:best,null);
      }
      const target=a.target;
      if(target&&dist(target,game.player)>150&&(!pointer.enabled||dist(target,pointer)>85)){dx=target.x-a.x;dy=target.y-a.y}
    }
    ...
    const speed=(a.dash>0?680:a.bot?65:260)*(game.event?.id==='speed'&&game.eventTime>0?1.35:1);move(a,dx,dy,speed,dt)
  }
  function update(dt){const g=game;if(!g?.playing||g.paused)return;g.time=Math.max(0,g.time-dt);g.elapsed+=dt;g.eventClock-=dt;g.eventTime=Math.max(0,g.eventTime-dt);g.shake=Math.max(0,g.shake-dt);if(g.eventTime===0)ui.event.classList.remove('show');if(g.eventClock<=0)startEvent();if(g.time<=0){finish();return}
    ...
    for(let i=g.cakes.length-1;i>=0;i--){const c=g.cakes[i];c.phase+=dt*3;let collector=null;const gap=dist(g.player,c);if(pointer.enabled&&gap<95&&gap>1){const pull=Math.min(gap,320*dt);c.x+=(g.player.x-c.x)/gap*pull;c.y+=(g.player.y-c.y)/gap*pull}for(const a of g.actors)if(a.pickupCooldown<=0&&(!a.bot||(g.elapsed>=5&&dist(c,g.player)>150&&(!pointer.enabled||dist(c,pointer)>85)))&&dist(a,c)<a.radius+15){collector=a;break}if(collector){award(collector,c);g.cakes.splice(i,1)}}
    for(const a of g.actors)for(const b of g.actors)if(a!==b&&dist(a,b)<a.radius+b.radius+4)collide(a,b);
    for(const c of g.clouds){c.phase+=dt;c.x+=c.vx*dt;c.y+=c.vy*dt;if(c.x<45||c.x>W-45)c.vx*=-1;if(c.y<45||c.y>H-45)c.vy*=-1;for(const a of g.actors)if(a!==g.player&&a.dash<=0&&a.invuln<=0&&dist(c,a)<43){a.stun=.45;a.invuln=1.5;if(!a.bot&&a!==game.player){floatText(a.x,a.y,'乌云减速！','#dfc1f0');beep(170,.22,'sawtooth',.035)}}}
  ...
  function updateHud(){const g=game;if(!g)return;ui.time.textContent=Math.ceil(g.time);ui.score.textContent=g.player.score.toLocaleString();ui.combo.textContent=`×${Math.max(1,g.player.combo)}`;ui.rank.textContent=`#${[...g.actors].sort((a,b)=>b.score-a.score).indexOf(g.player)+1}`;ui.touchDash.disabled=g.player.cool>0||!g.playing;ui.dashStatus.textContent=g.player.cool>0?`加速 ${g.player.cool.toFixed(1)} 秒`:'加速准备就绪';}
  ...
  function draw(){ctx.clearRect(0,0,W,H);const sky=ctx.createLinearGradient(0,0,W,H);sky.addColorStop(0,'#292048');sky.addColorStop(1,'#403059');ctx.fillStyle=sky;ctx.fillRect(0,0,W,H);circle(780,118,65,'#ffdb9728');circle(780,118,51,'#ffe9ad');circle(761,101,10,'#e9ca8b55');circle(801,130,15,'#e9ca8b55');const t=performance.now()/1000;ctx.save();if(game?.shake)ctx.translate(rand(-3,3),rand(-3,3));for(let i=0;i<36;i++)circle((i*237+61)%W,(i*169+53)%H,1.3+Math.sin(t*2+i)*.5,'#fff3c58c');
    ...
    }else{ctx.textAlign='center';ctx.font='bold 38px sans-serif';ctx.fillStyle='#ffe1a19a';ctx.fillText('🌕  🥮  🌕',480,330)}ctx.restore()}
  function frame(now){const dt=Math.min(.05,(now-last)/1000||0);last=now;update(dt);draw();requestAnimationFrame(frame)}
  function setPoint(e){const rect=canvas.getBoundingClientRect();pointer.x=clamp((e.clientX-rect.left)/rect.width*W,0,W);pointer.y=clamp((e.clientY-rect.top)/rect.height*H,0,H);if(game?.playing){const target=game.cakes.filter(c=>dist(c,pointer)<65).sort((a,b)=>dist(a,pointer)-dist(b,pointer))[0];if(target){pointer.x=target.x;pointer.y=target.y}}}
  ...
  document.querySelectorAll('.mode').forEach(btn=>btn.addEventListener('click',()=>{mode=btn.dataset.mode;document.querySelectorAll('.mode').forEach(b=>b.classList.toggle('selected',b===btn));ui.hint.textContent=mode==='duo'?'1P：WASD + 空格 · 2P：方向键 + 右 Shift（建议电脑双人）':'iPad：点月饼或拖动手指，按 ⚡ 加速';beep(570,.08)}));
  ui.start.addEventListener('click',()=>game?.paused?pause():start());ui.pause.addEventListener('click',pause);ui.dash.addEventListener('click',()=>dash(game?.player));ui.touchDash.addEventListener('pointerdown',e=>e.stopPropagation());ui.touchDash.addEventListener('click',e=>{e.stopPropagation();dash(game?.player)});ui.sound.addEventListener('click',()=>{if(!muted&&music.paused&&game?.playing&&!game.paused){playMusic();return}muted=!muted;music.muted=muted;ui.sound.textContent=muted?'🔇':'🔊';ui.sound.setAttribute('aria-label',muted?'开启声音':'关闭声音');if(!muted&&game?.playing&&!game.paused)playMusic()});
  const music=$('musicPlayer');
  const tracks=[
    'https://kunruiw1991.github.io/lumipop-kids-tv/videos/dh_02_golden_lyrics.mp4',
    'https://kunruiw1991.github.io/lumipop-kids-tv/videos/dh_01_soda_pop.mp4'
  ];
  let track=0;music.src=tracks[track];music.volume=.55;
  function playMusic(){if(muted)return;music.play().then(()=>ui.sound.textContent='🔊').catch(()=>{ui.sound.textContent='🔈';ui.sound.setAttribute('aria-label','点按开启音乐')})}
  function nextMusic(){track=(track+1)%tracks.length;music.src=tracks[track];if(game?.playing&&!game.paused)playMusic()}
  $('musicNext').addEventListener('click',nextMusic);music.addEventListener('ended',nextMusic);
  music.addEventListener('error',()=>{ui.sound.textContent='🔈';ui.sound.setAttribute('aria-label','音乐暂时无法加载，点按重试')});
  document.addEventListener('visibilitychange',()=>{if(document.hidden&&game?.playing&&!game.paused)pause()});
